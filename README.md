...
- Both orchestrators set `@@location = 'us-east1'` to match loyalty dataset region and avoid cross-region job errors.
...
- Overhead split behavior: `Gerente` remains store-specific, while `RH` and `Financeiro` are shared across all stores (`store_prefix = ALL`).
