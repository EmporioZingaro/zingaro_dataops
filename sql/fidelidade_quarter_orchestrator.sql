...
IF cashback_exists THEN
  BEGIN
    EXECUTE IMMEDIATE FORMAT(
      "SELECT COUNT(*) > 0 FROM `%s.%s.INFORMATION_SCHEMA.COLUMNS` WHERE table_name = 'cashback' AND column_name = 'quarter_id'",
      project_id,
      dataset_id
    ) INTO cashback_exists;
...
CREATE OR REPLACE TABLE %s
OPTIONS (
  description = 'Current commissions by store/seller for most recently closed quarter, with validated overhead split (RH/Financeiro shared across stores)'
) AS
WITH
  RecipientSplit AS (
    SELECT * FROM UNNEST([
